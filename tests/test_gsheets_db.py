    ...
    def get_test_schema(encrypted_fields=None):
        ...
        return Schema(
            ...
            [
                ...
                Field(
                    "notes", FieldType.STRING, required=True, encrypted="notes" in encrypted_fields
                ),
            ],
        )
...
async def sheet_manager(encryption_key):
    ...
    manager = SheetManager(
        connection=connection, schema=TestSetup.get_test_schema(), encryption_key=encryption_key
    )
...
async def test_bulk_operations(sheet_manager):
    ...
    update_result = await sheet_manager.update(
        filters={"name": {"$regex": "Test User"}}, updates={"salary": 60000.00}
    )
...
async def test_validation_rules(sheet_manager):
    ...
    with pytest.raises(ValueError):
        await sheet_manager.insert({"id": 1, "email": "test@example.com"})
