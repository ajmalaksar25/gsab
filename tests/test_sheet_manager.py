...
async def test_sheet_manager_complex_operations(sheet_manager):
    ...
    test_data = [{"id": i, "name": f"User {i}", "age": 20 + i} for i in range(1, 4)]
    ...
    results = await sheet_manager.read({"age": {"$gt": 21}, "name": {"$regex": "User"}})
    assert len(results) > 0

    # Test update with complex filter
    await sheet_manager.update({"age": {"$gt": 21}}, {"name": "Updated User"})
