...
async def home(request: Request):
    ...
    return templates.TemplateResponse(
        "index.html", {"request": request, "is_connected": app_state.connection is not None}
    )
...
async def create_sheet(
    ...
    try:
        ...
        app_state.sheet_manager = SheetManager(
            app_state.connection, schema, encryption_key=schema_definition.get("encryption_key")
        )

        sheet_id = await app_state.sheet_manager.create_sheet(title)
        return {"status": "success", "sheet_id": sheet_id, "message": f"Created sheet: {title}"}
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e)) from e
